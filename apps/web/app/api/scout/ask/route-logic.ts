...
interface HistoryReviewContext {
  skill: string
  skillLabel: string
  prompt: string
...
export function parseHistoryReviewContext(
  value: string | null | undefined
): HistoryReviewContext | null {
  ...
  if (
    input.k !== "saved-mistake" ||
    (input.x !== "english" && input.x !== "math" && input.x !== "reading")
  ) {
    return null
  }
  const section = input.x as HistoryReviewContext["section"]
  const context: HistoryReviewContext = {
    skill: text(input.g, 48),
    skillLabel: text(input.s, 64),
    prompt: text(input.p, 220),
    selectedChoiceText: text(input.a, 280),
    correctChoiceText: text(input.c, 280),
    rationale: text(input.r, 420),
    section,
  }
...
function parsePreferences(value: unknown): ScoutExplanationPreferences {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    throw new RangeError("AlexACT explanation preferences are malformed.")
  }
...
async function getDiagnostic(request: NextRequest) {
...
function mrKimGroundingFacts(input: {
  ...
  diagnostic?: Awaited<ReturnType<typeof getDiagnostic>>
  ...
  const historyReview =
    input.request.screen === "history"
      ? parseHistoryReviewContext(input.request.selectedText)
      : null
  if (historyReview) {
    facts.push(
      `The saved mistake is from ${historyReview.skillLabel}.`,
      `The saved prompt is: ${historyReview.prompt}`,
      `The learner chose: ${historyReview.selectedChoiceText}`,
      `The reviewed correct answer is: ${historyReview.correctChoiceText}`,
      `The reviewed rationale is: ${historyReview.rationale}`
    )
  }
  if (
    input.learning &&
    input.request.screen !== "badges" &&
    input.request.screen !== "diagnostic-review" &&
    input.request.screen !== "history"
  ) {
  ...
  if (
    input.answer.receipt.assistanceMode === "review" &&
    input.diagnostic?.result
  ) {
    const question = input.request.questionId
      ? input.diagnostic.form.questions.find(
          (candidate) => candidate.id === input.request.questionId
        )
      : null
    const feedback = input.request.questionId
      ? input.diagnostic.result.feedback.find(
          (candidate) => candidate.questionId === input.request.questionId
        )
      : null
    facts.push(
      "The learner has submitted the diagnostic and missed-question review is unlocked.",
      ...(question
        ? [
            `The reviewed diagnostic question is: ${question.prompt}`,
            ...question.choices.map(
              (choice) => `Choice ${choice.id}: ${choice.text}`
            ),
          ]
        : []),
      ...(feedback
        ? [
            `The learner's original choice was ${feedback.selectedChoiceId}.`,
            `The correct choice is ${feedback.correctChoiceId}.`,
            `The reviewed rationale is: ${feedback.rationale}`,
          ]
        : [])
    )
  }
...
export function answerFor(input: {
  ...
  const historyReview =
    request.screen === "history"
      ? parseHistoryReviewContext(request.selectedText)
      : null
  const exam = request.screen === "lab" ? input.exam : null
  const diagnostic =
    request.screen === "diagnostic-review" ? (input.diagnostic ?? null) : null
  const examMode = diagnostic
    ? "review"
    : exam
  ...
  const diagnosticQuestion = request.questionId
    ? diagnostic?.form.questions.find(
        (question) => question.id === request.questionId
      )
    : undefined
  if (
  ...
  const activeQuestion =
    examMode === "study"
      ? learningQuestion
      : (diagnosticQuestion ?? examQuestion)
  const questionId = activeQuestion?.id ?? null
  const skillId = historyReview
    ? historyReview.skill
    : request.screen === "badges"
      ? null
      : (diagnosticQuestion?.primarySkill ??
        examQuestion?.primarySkill ??
        learningQuestion?.skill ??
        learning?.todaySkill ??
        null)
  const attempted = historyReview
    ? true
    : examMode === "study"
      ? Boolean(
      ...
      : diagnostic
        ? Boolean(
            questionId &&
            diagnostic.result?.feedback.some(
  ...
  const review = questionId
    ? (diagnostic?.result?.feedback.find(
  ...
  const corpus = [
    ...
    ...(historyReview
      ? [
          historyReview.prompt,
          historyReview.selectedChoiceText,
          historyReview.correctChoiceText,
          historyReview.rationale,
        ]
      : []),
    ...(activeQuestion
      ? [
  ...
  if (historyReview) {
    summary = `Let’s review this ${historyReview.skillLabel} miss.`
    explanation = historyReview.rationale
    example = `Your answer: ${historyReview.selectedChoiceText}\nCorrect answer: ${historyReview.correctChoiceText}`
    nextAction =
      "Explain the rule back in your own words, then ask me for one similar example."
    source = "Saved AlexACT mistake history"
    delivery = "reviewed-rule"
  } else if (previous && request.screen !== "badges") {
    ...
    if (/another example/.test(lower)) {
      ...
      explanation = `${previous.answer.explanation} It matters because AlexACT uses that evidence to choose what you practice next without pretending one answer proves everything.`
    ...
    explanation =
      "The report shows raw accuracy and average time per answered question. It does not update Lessons, My Schedule, or the skill web."
    nextAction =
    ...
    source = diagnostic
      ? `Scored diagnostic review for ${review.questionId}`
      : `Scored Timed Practice review for ${review.questionId}`
    ...
    if (badgeProgress) {
      ...
      explanation =
        "Open a lesson once so AlexACT can load the server-owned progress used by Badges."
      example = null
      nextAction =
    ...
    summary = calibration
      ? `Here, “margin of error” means AlexACT’s estimate is still about ±${calibration.estimate.standardError.toFixed(2)} on its internal scale.`
      : "Here, “margin of error” means AlexACT’s estimate is still shaky."
    explanation = calibration
      ? `A smaller ± number means the estimate is settling down. The shaded band shows the middle 80% of values AlexACT considers plausible, from ${calibration.estimate.interval80.low.toFixed(2)} to ${calibration.estimate.interval80.high.toFixed(2)} on its internal scale. These are not ACT score points, and the app uses preset question difficulty rather than national calibration.`
      : "A smaller ± number means AlexACT is less unsure. The shaded band shows the middle 80% of values AlexACT considers plausible. Both use AlexACT’s internal scale, not ACT score points, and AlexACT cannot read a current Quick Check session from this request."
    example = calibration
    ...
    if (request.screen === "plan") {
      ...
      explanation = studyPlan
        ...
        : "Open “Why this is scheduled” on My Schedule."
      example = null
      nextAction =
      ...
      explanation =
        "AlexACT chose it from your recent scored answers and where more practice will help most."
      example = null
      nextAction =
    ...
    summary = selectedState
      ...
      : "AlexACT does not have enough scored evidence for a skill estimate yet."
    ...
    summary = studyPlan
      ...
      : "AlexACT cannot read the dated calendar in this answer."
    explanation = studyPlan
      ? `${studyPlan.forecast.scheduledMinutes} minutes are scheduled before test day. You can change the available days or minutes from My Schedule.`
      : "Open My Schedule to inspect the calendar. The assistant currently has only the learning-session context."
  ...
  const personalized = applyPreferences(
    {
      ...
      technical:
        request.screen === "badges"
          ...
          : "This response used the current lesson, result, or plan fields named under Source. AlexACT did not read the rest of the visible screen.",
      nextAction,
      source,
      mode: "grounded",
    },
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!question) throw new RangeError("Ask Mr. Kim a question first.")
    ...
    const scoutRequest: ScoutAskRequest = {
      ...
      selectedText:
        text(body.selectedText, screen === "history" ? 1_600 : 400) || null,
    }
    const [learning, exam, diagnostic, calibration, studyPlan] =
      await Promise.all([
        getLearning(request),
        scoutRequest.screen === "lab"
          ? getExam(request)
          : Promise.resolve(null),
        scoutRequest.screen === "diagnostic-review"
          ? getDiagnostic(request)
          : Promise.resolve(null),
