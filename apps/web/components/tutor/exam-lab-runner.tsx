    ...
    <nav aria-label="Question navigator">
      ...
      <div className="mt-3 grid grid-cols-6 gap-2 lg:grid-cols-5">
        {questions.map((question) => {
          const globalIndex = session.questions.findIndex((item) => item.id === question.id)
          ...
          return (
            <button
              ...
              className={cn(
                ...
                current && "border-foreground bg-[var(--coach-surface)] shadow-[2px_2px_0_var(--foreground)]"
              )}
              onClick={() => onMove(globalIndex)}
            >
              {globalIndex + 1}
              {response?.flagged ? <BookmarkIcon className="absolute -top-1 -right-1 size-3 fill-[var(--scout-coral)] text-[var(--scout-coral)]" /> : null}
            </button>
          )
        })}
      </div>
      <div className="mt-4 flex flex-wrap gap-x-4 gap-y-2 text-[0.68rem] text-muted-foreground">
        <span className="inline-flex items-center gap-1"><span className="size-2 bg-primary" /> Answered</span>
        <span className="inline-flex items-center gap-1"><BookmarkIcon className="size-3 text-[var(--scout-coral)]" /> Flagged</span>
      </div>
    </nav>
  )
}
...
export function ExamLabRunner({
  ...
  const response = session.progress.responses[question.id]
  const navigationSection = session.progress.phase === "review" ? question.section : session.progress.currentSection
  const sectionQuestions = navigationSection === "mixed"
    ? session.questions
    : session.questions.filter((item) => item.section === navigationSection)
  const sectionPosition = sectionQuestions.findIndex((item) => item.id === question.id)
  const progress = Math.round(((sectionPosition + 1) / sectionQuestions.length) * 100)
  const timeCritical = timeLeft <= 60
  const sectionName = navigationSection === "mixed"
    ? "Mixed sprint"
    ...
    <main className="min-h-[calc(100svh-5rem)] bg-[var(--rail)]">
      <header className="sticky top-0 z-30 border-b-2 border-foreground bg-background">
        <div className="mx-auto grid max-w-[100rem] grid-cols-[minmax(0,1fr)_auto] items-center gap-4 px-4 py-3 sm:px-7 lg:grid-cols-[1fr_auto_1fr]">
          <div>
            ...
            <p className="mt-1 truncate font-heading text-xl font-bold">{session.title}</p>
          </div>
          <div className={cn("flex items-center gap-3 border-2 border-foreground px-4 py-2", timeCritical && "border-[var(--scout-coral)] bg-[var(--coach-surface)] text-[var(--destructive)]")}>
            <Clock3Icon className={cn("size-5", timeCritical && "animate-pulse")} aria-hidden="true" />
            <div>
              <p className="ink-label">{sectionName}</p>
              <p className="font-mono text-xl font-black tabular-nums">{formatTime(timeLeft)}</p>
            </div>
          </div>
          <div className="col-span-2 flex items-center justify-between gap-4 lg:col-span-1 lg:justify-self-end">
            <span className={cn("inline-flex items-center gap-2 text-xs font-semibold", saveStatus === "error" ? "text-destructive" : "text-muted-foreground")}>
              {saveStatus === "saved" ? <CheckCircle2Icon className="size-4 text-primary" /> : <SaveIcon className="size-4" />}
              {saveStatus === "saved" ? "Saved" : saveStatus === "saving" ? "Saving…" : "Save failed"}
            </span>
            <Button type="button" variant="outline" size="sm" onClick={onEndSection} disabled={busy}>
              {session.progress.phase === "review" ? "Return to review" : timeLeft === 0 ? "Time expired" : "End section"}
      ...
      <div className="mx-auto grid max-w-[100rem] lg:grid-cols-[minmax(20rem,0.9fr)_minmax(26rem,1.1fr)_16rem]">
        <section className="hidden min-w-0 border-b bg-background px-5 py-7 sm:px-8 lg:block lg:min-h-[calc(100svh-10rem)] lg:border-r lg:border-b-0">
          {question.stimulus ? (
            <>
              <p className="ink-label text-muted-foreground">{question.passageTitle ?? `${sectionName} stimulus`}</p>
              <article className="mt-5 whitespace-pre-line text-base leading-8">{question.stimulus}</article>
            </>
          ) : (
            <div className="flex min-h-[30rem] items-center justify-center">
              <div className="max-w-xs text-center">
                <p className="font-heading text-4xl font-bold">No passage for this item.</p>
                <p className="mt-3 text-sm leading-6 text-muted-foreground">Use this space to sketch, calculate, or slow down the first decision.</p>
              </div>
            </div>
          )}
        </section>
        ...
        <section className="min-w-0 bg-background px-5 py-7 sm:px-8 sm:py-9">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <p className="ink-label text-primary">Question {session.progress.currentIndex + 1} of {session.questions.length}</p>
            <Button
              type="button"
            ...
            >
              <BookmarkIcon data-icon="inline-start" className={cn(response?.flagged && "fill-current")} />
          ...
          {question.stimulus ? (
            <details className="mt-5 border-y-2 border-foreground py-4 lg:hidden">
              ...
              <article className="mt-4 whitespace-pre-line text-sm leading-7">{question.stimulus}</article>
            </details>
          ) : null}
          {question.lineReference ? <p className="mt-3 font-mono text-xs font-bold text-muted-foreground">{question.lineReference}</p> : null}
          <h1 className="mt-5 font-heading text-3xl leading-tight font-bold tracking-[-0.02em] sm:text-4xl">{question.prompt}</h1>
            ...
            {question.choices.map((choice, index) => (
              <label
                ...
                className={cn(
                  ...
                  response?.choiceId === choice.id && "border-primary bg-secondary"
                )}
              >
          ...
          <fieldset className="mt-7 border-y py-5">
            <legend className="ink-label text-muted-foreground">How sure are you?</legend>
            <div className="mt-3 grid grid-cols-3 gap-2">
              {(["guess", "unsure", "sure"] as const).map((confidence) => (
                <Button
                  key={confidence}
                  type="button"
                  variant={response?.confidence === confidence && response.choiceId ? "default" : "outline"}
          ...
          <div className="mt-7 flex items-center justify-between gap-3">
            <Button
              type="button"
              variant="outline"
              onClick={() => onMove(session.questions.findIndex((item) => item.id === sectionQuestions[sectionPosition - 1]?.id))}
        ...
        <aside className="border-t bg-[var(--rail)] px-5 py-6 lg:sticky lg:top-[8.5rem] lg:h-[calc(100svh-8.5rem)] lg:overflow-y-auto lg:border-t-0 lg:border-l">
          <QuestionNavigator session={session} questions={sectionQuestions} onMove={onMove} />
          <div className="mt-7 border-t pt-5 text-xs leading-5 text-muted-foreground">
            <p className="font-semibold text-foreground">No correctness during the run.</p>
            <p className="mt-1">Scout scores only after submission so the simulation stays honest.</p>
          </div>
        </aside>
      </div>
    </main>
