...
const questions = [
  ["e1", "english", "boundaries", "a"], ["e2", "english", "concision", "b"],
  ["m1", "math", "linear", "a"], ["m2", "math", "geometry", "b"],
  ["r1", "reading", "inference", "a"], ["r2", "reading", "purpose", "b"],
].map(([id, section, skill, correct]) => ({
  id, version: 1, section, category: "C", primarySkill: skill, skillLabel: skill, difficulty: "medium",
  prompt: `Prompt ${id}`, choices: ["a", "b", "c", "d"].map((choice) => ({ id: choice, text: choice })),
  expectedSeconds: 45, format: "standalone", correctChoiceId: correct, rationale: `Reason ${id}`,
  content: { status: "published", license: "original", reviewer: "test", reviewedAt: "2026-07-12" },
}));
const form = { id: "lab", version: "1", mode: "rapid", title: "Lab", estimatedMinutes: 10, blueprint: [], questions } as DiagnosticFormSecure;

async function withRepo<T>(run: (repo: FileExamLabRepository) => Promise<T>) {
  const dir = await mkdtemp(join(tmpdir(), "exam-lab-"));
  try { return await run(new FileExamLabRepository(join(dir, "lab.json"))); }
...
describe("FileExamLabRepository", () => {
  it("starts a public sprint and withholds keys", async () => {
    await withRepo(async (repo) => {
      ...
      expect(JSON.stringify(started.payload.questions)).not.toContain("correctChoiceId");
  ...
  it("autosaves answer confidence, time, and flags", async () => {
    await withRepo(async (repo) => {
      const started = await repo.start(form, { mode: "section", section: "english" });
      const saved = await repo.save(started.sessionId, form, {
        currentIndex: 1, phase: "questions",
        responses: { e1: { choiceId: "a", confidence: "sure", flagged: true, elapsedSeconds: 31 } },
      });
      expect(saved.progress.responses.e1).toMatchObject({ confidence: "sure", flagged: true, elapsedSeconds: 31 });
    });
  });
  ...
  it("advances core sections and finalizes an idempotent report", async () => {
    await withRepo(async (repo) => {
      ...
      expect((await repo.finalize(started.sessionId, form)).result).toEqual(result.result);
    });
  });
});
