...
function stringList(value: unknown, field: string) {
  if (!Array.isArray(value) || value.length !== 2) {
    throw new TypeError(
      `AI debrief field ${field} must contain exactly two items.`,
    );
  }
...
function validateDebrief(value: unknown, model: string): ExamDebrief {
  if (!value || typeof value !== "object")
    throw new TypeError("AI debrief is not an object.");
              ...
              },
              {
                ...
                content: JSON.stringify({
                  ...
                  requiredJson: {
                    ...
                    summary:
                      "2-3 simple sentences about correct answers and pacing",
                    wins: ["exactly two specific things that went well"],
                    priorities: ["exactly two specific things to improve"],
                    nextAction:
                      "one concrete lesson, practice, or pacing action",
                  },
                }),
              },
            ],
