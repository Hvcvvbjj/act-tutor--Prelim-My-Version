...
| Cross-skill/prerequisite confusion | AlexACT checks a defined prerequisite graph and recommends prerequisite repair before returning to the target skill. |
...
| Learner correction | “AlexACT got this wrong about me” records a bounded correction, preserves original evidence, reruns the next-skill policy, and keeps an audit trail. |
| Mastery versus certainty | AlexACT Lab displays the skill estimate and certainty as separate values with plain-language meaning. |
...
| Timed-practice guidance | AlexACT explains when a reviewed timed form is more useful than another short skill set; it does not turn skill estimates into a readiness gate. |
| Parallel-form assessment | Repair, retention, challenge, and recovery choose the least-exposed reviewed item form; AlexACT says when the local bank cannot provide a truly unseen item. |
...
| ACT blueprint | AlexACT Lab shows current questions, minutes, average pace, optional Science, and official reporting-category percentages. |
...
| Effort versus progress | AlexACT compares trusted answers with average readiness and names stable skills. |
...
| Feature | Product behavior |
| --- | --- |
| Coach Brief | A read-only learner summary names the strongest demonstrated skill, main misconception, evidence level, current and next missions, an offline teaching prompt, and what AlexACT still does not know. There is no unauthenticated teacher-edit surface. |
...
| Imported group-metric viewer | Consented local JSON rows supply precomputed selection, error, and stopping metrics. AlexACT averages and displays them but does not derive or verify a fairness audit from raw events. |
...
| Data export/delete | A JSON export is downloadable and all session/local AlexACT data can be permanently deleted. |
...
## AlexACT as the product-wide plain-English layer

- Persistent labeled **Ask Mr. Kim** launcher on desktop; a compact AlexACT mascot is docked inside the mobile navigation. The assistant opens as a side panel or bottom sheet.
